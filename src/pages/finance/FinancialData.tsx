...
export const FinancialData = ({ t, lang }: Props) => {
  ...
  // ── "i" tooltip — React Portal (document.body) ile tablo stacking context'inin
  //    tamamen dışında; hiçbir hücre/rozet üstüne binemez. ─────────────────────
  const InfoTip = ({ termKey }: { termKey: string }) => {
    ...
    return (
      <span className="fin-i" style={{ display: 'inline-flex', marginLeft: 5, opacity: 0.28, transition: 'opacity 0.12s' }}
        onMouseEnter={(e) => { const r = (e.currentTarget as HTMLElement).getBoundingClientRect(); setPos({ x: r.left + r.width / 2, y: r.top }); }}
        onMouseLeave={() => setPos(null)}>
        <Icon name="info" size={12} color={t.tx3} />
        {pos && createPortal(
          <div style={{ position: 'fixed', left: Math.max(8, Math.min(pos.x - 125, window.innerWidth - 258)), top: pos.y - 8, transform: 'translateY(-100%)', width: 250, background: t.tx, color: t.bg, borderRadius: 8, padding: '9px 12px', fontSize: 11, lineHeight: 1.45, boxShadow: '0 6px 20px rgba(0,0,0,0.35)', zIndex: 99999, fontWeight: 400, whiteSpace: 'normal', pointerEvents: 'none' }}>
            {txt}
          </div>,
          document.body,
        )}
