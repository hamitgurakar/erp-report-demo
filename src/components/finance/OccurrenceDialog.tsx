...
// SECTION C.3 — occurrence taşı/düzenle + iptal kapsam dialogu (Google Calendar modeli)
export const OccurrenceDialog = ({ t, lang, series, recurrenceId, mode, onClose }: { t: Theme; lang: Lang; series: RecurringSeries; recurrenceId: string; mode: 'edit' | 'cancel'; onClose: () => void }) => {
  const en = lang === 'en';
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 260, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: t.cd, borderRadius: 14, padding: 22, width: 440, maxWidth: '100%', border: `1px solid ${t.bd}`, boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: t.tx, marginBottom: 3 }}>{mode === 'edit' ? L('Oluşumu Taşı / Düzenle', 'Move / Edit Occurrence') : L('Ödemeyi İptal Et', 'Cancel Payment')}</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
          ...
          <button onClick={mode === 'edit' ? applyEdit : applyCancel} style={{ ...btnP, background: mode === 'cancel' ? t.rd : '#4F46E5' }}>{mode === 'edit' ? L('Uygula', 'Apply') : L('Onayla', 'Confirm')}</button>
        </div>
      </div>
    </div>
  );
